...
sub _test_shell_cmds ( $shell, $buildin, @args ) {
    ...
    foreach my $queue (@args) {
        foreach my $line ( $queue->@* ) {    # check for completeness
...
sub _replace_variables ( $buildin, $variables, @queue ) {
    ...
    foreach my $line (@queue) {
        ...
        die 'ERROR: cannot execute macro, required variables not set' if ( $failed_variable eq '1' );    # otherwise its a string
...
sub _curry_shell ($core) {
    ...
    # although right now, there is only ONE 'savefile' supported. so one would really need to chroot multiple times to make use of this.
    ...
    return sub ($query) {
        ...
        my ( $queue, $toexecute ) = _split_macro_queue( $query->() );         # split queue into before and after first CONTINUE
