...
# useful to trace bugs relating wrong assumptions about data.
# this module is old and full of little quirks, but performs well enough
# state variables were introduced long after this, so it lacks the capability to resolve CODE references
##########################################################################################
...
sub _query_wrapper ( $query, $args ) {
    ...
    my $result = $query->();    # we get all the data, so we avoid die if data is not found;
...
sub list_keys ( $data, @keys ) {
    ...
    my $result = _query_wrapper( $query->{DATA}, $args );    # normal query interface
...
sub import_view ($core) {
    ...
    my $struct = {
        view => {
            ...
            HELP => [ 'usage:', 'use \'help\' to get a command overview, then type', '', 'view <full cmd> [args]', '', 'to query a commands dataset.', ],
            DATA => {}
        },
        keys => {
            CMD => sub (@args) {
